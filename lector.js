...
fs.readFile("database.txt", "utf8", (err, data) => {
  ...
  function buscarPalabraEnObjeto(palabra, objeto) {
    if (objeto.hasOwnProperty(palabra)) {
      console.log(
        `El número correspondiente a la palabra "${palabra}" es: ${objeto[palabra]}`
      );
      ...
      //aqui se verifica si la palabra empieza con "'"
      arregloFinalTOkenizado.push(1001);
    } else {
      arregloFinalTOkenizado.push(1000);
      console.log(
        `La palabra "${palabra}" no está reservada; por lo tanto, se tokeniza con el valor 1000.`
      );
    }
  }
 ...
 */
  ...
  console.log(
    "aqui se muestra las o la consultas a ejecutar:",
    "\n",
  ...
  for (let i = 0; i < cadena_split.length; i++) {
    ...
    console.log(
      "--------------------------------------",
      "\n",
      "este es el arreglo tokenizado:",
      JSON.stringify(arregloFinalTOkenizado),
      "\n",
      "-----------------------------"
    );
    ...
    if (indice === 0) {
      ...
      console.log(
        " Error ,consulta incorrecta la palabra SELECT no se encontro"
      );
      renglonesIncorrectos.push(
        "Error, consulta incorrecta la palabra SELECT no se encontro"
      );
    }
    /*
    ...
    if (indice === 1 && arregloFinalTOkenizado[indice] !== 3) {
      //ESTO SE LLEVARA ACABO SI SE ENCONTRO SELECT POSTERIOR MENTE y la pocicion 1 no es una ,
      if (arregloFinalTOkenizado[indice] === 7) {
        ...
        if (arregloFinalTOkenizado[indice] === 309) {
          ...
          if (arregloFinalTOkenizado[indice] === 1000) {
            ...
            if (arregloFinalTOkenizado[indice] === 6) {
              ...
              renglonesIncorrectos.push(
                " Error, no se termino la consulta con ';'"
              );
            }
          } else {
            console.log(
              "ERROR no se puede utilizar una palabra reservada despues de FROM"
            );
            renglonesIncorrectos.push(
              "ERROR no se puede utilizar una palabra reservada despues de FROM"
            );
          ...
          renglonesIncorrectos.log("Error se esperaba FROM en la posicion", [
            indice,
          ]);
        }
        ...
        renglonesIncorrectos.push(
          "Error se esperaba un '*' o 'una palabra no reservada"
        );
      }
      ...
      if (contieneFrom) {
        ...
        if (
          arregloFinalTOkenizado[indice - 2] == 3 ||
          arregloFinalTOkenizado[indice - 2 == 655]
        ) {
          console.log(
            "Error la consulta es incorrecta ya que no debe un 'SELECT' o una ',' antes del FROM "
          );
          renglonesIncorrectos.push(
            "Error la consulta es incorrecta ya que no debe ir  una 'palabra reservada' antes del FROM "
          );
        }
          ...
          if (trabajarConAsterisco) {
            if (arregloFinalTOkenizado[indice] === 1000) {
              ...
              if (arregloFinalTOkenizado[indice] === 6) {
                ...
                renglonesIncorrectos.push(
                  "Error consulta no terminada con ';'"
                );
              }
            } else {
              console.log(
                "error no se puede trabajar con una palabra reservada despues de FROM"
              );
              renglonesIncorrectos.push(
                "error no se puede trabajar con una palabra reservada despues de FROM"
              );
      ...
      console.log(
        "error se escribio una ',' antes de '*' o 'palabra no reservada' "
      );
    }
  ...
  const tokenArchivo = arregloTokentxt
